...
if($_SESSION['randnumber'] == $_POST['randnumber'])
{
    if(isset($_POST['submit']))
    {
        ...
        $sql_verify = "SELECT password FROM customer WHERE customer_id='$_SESSION[customerid]'";
        $qsql_verify = mysqli_query($con, $sql_verify);
        $user = mysqli_fetch_assoc($qsql_verify);

        // Verify old password
        if(password_verify($_POST['oldpassword'], $user['password']))
        {
            // Hash new password
            $new_hashed_password = password_hash($_POST['newpassword'], PASSWORD_DEFAULT);

            // Update password
            $sql_update = "UPDATE customer SET password='$new_hashed_password' WHERE customer_id='$_SESSION[customerid]'";
            $qsql_update = mysqli_query($con, $sql_update);
