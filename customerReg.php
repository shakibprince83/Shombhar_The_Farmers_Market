...
if($_SESSION['randnumber']  == $_POST['randnumber'])
{
	if(isset($_POST['submit']))
	{
		if(isset($_GET['editid']))
		{
			// Email check not needed for update
		...
		{	
			...
			$check_email = $con->prepare("SELECT * FROM customer WHERE email_id = ?");
			$check_email->bind_param("s", $_POST['email_id']);
			$check_email->execute();
			...
			$sql="INSERT INTO `customer`( `customer_name`, `address`, `country_id`, `state_id`, `city_id`, `pincode`, `contact_no`, `mobile_no`, `email_id`, `password`, `customer_type`, `status`) VALUES ('$_POST[customername]','$_POST[address]','$_POST[country]','$_POST[state]','$_POST[city]','$_POST[pincode]','$_POST[cntctnum]','$_POST[mblnum]','$_POST[email_id]','$hashed_password','$_POST[customertype]','$_POST[status]')";	
			if(!mysqli_query($con,$sql))
			{
...
var passwordExp = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]).{10,}$/;
...
if(document.frmcstreg.customername.value == "")
	...
	else if(document.frmcstreg.mblnum.value.length != 11)
	{
		alert("Mobile Number should contain 11 digits..");
		document.frmcstreg.mblnum.focus();
		return false;
	}
	...
	else if(document.frmcstreg.password.value.length < 10)
	{
